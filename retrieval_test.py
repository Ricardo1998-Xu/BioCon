...
from model import CodeT5Model as Model
...
from transformers import (WEIGHTS_NAME, get_linear_schedule_with_warmup, AutoConfig, AutoModelForSequenceClassification,
                          AutoTokenizer, T5Config, T5ForConditionalGeneration, RobertaConfig,
                          RobertaForSequenceClassification, RobertaTokenizer, CodeGenConfig, CodeGenModel,
                          )
...
MODEL_CLASSES = {
    'codebert': (RobertaConfig, RobertaForSequenceClassification, RobertaTokenizer),
...
def plot_rank_distribution(ranks, save_path):
    plt.figure()
    ...
    plt.savefig(os.path.join(save_path, "rank_distribution.png"), dpi=300)
    plt.close()


def plot_cdf(ranks, save_path):
    ranks = np.array(ranks)
    sorted_ranks = np.sort(ranks)

    yvals = np.arange(1, len(sorted_ranks)+1) / len(sorted_ranks)

    plt.figure()

    plt.plot(sorted_ranks, yvals)
    plt.xlabel("Rank")
    ...
    plt.savefig(os.path.join(save_path, "rank_cdf.png"), dpi=300)
    plt.close()


def plot_hit_k_curve(ranks, save_path, max_k=50):
    ranks = np.array(ranks)
    ...
    plt.savefig(os.path.join(save_path, "hit_k_curve.png"), dpi=300)
    plt.close()
...
def rank_candidates(model, tokenizer, args, query, candidates, device):
    scores = []
    ...
    with torch.no_grad():
        for i in range(0, len(candidates), batch_size):
            ...
            scores.extend(prob[:, 1].cpu().numpy())
...
def test(args, model, tokenizer):
    ...
    for item in tqdm(dataset):
        ...
        ranking = rank_candidates(model, tokenizer, args, query, candidates, args.device)
    ...
    results = {
        ...
        "Hit@20": round(hit20 / N, 4)
    }

    with open(os.path.join(results_path, 'ranks.txt'), 'a+') as f:
        f.write(str(all_ranks))

    plot_rank_distribution(all_ranks, results_path)
    plot_cdf(all_ranks, results_path)
    plot_hit_k_curve(all_ranks, results_path)
...
def main():
    ...
    parser.add_argument('--num_labels', type=int, default=2,
                        help='class num')
    parser.add_argument("--json_path", default='./PC.json', type=str, )
    parser.add_argument("--test_data_file", default='./data/BioCon/retrieval_cross-test.jsonl', type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")

    parser.add_argument("--output_dir", default='./weights/codet5p/FL-weight=5-lr0.00002-seed{seed}', type=str,
                        help="The output directory where the model predictions and checkpoints will be written.")
    parser.add_argument('--results_path', type=str,
                        default='./results/codet5p/FL-weight=5-lr0.00002-seed{seed}-retrieval-cross', help='save results path')
    ...
    parser.add_argument("--model_name_or_path", default='codet5p-220m', type=str,
                        help="The model checkpoint for weights initialization.")
    parser.add_argument("--tokenizer_name", default="codet5p-220m", type=str,
