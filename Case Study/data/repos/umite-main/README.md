...
Additional information is available in [our paper](https://doi.org/10.1093/bioinformatics/btag075)
...
which are automatically installed in the examples below.
For the alignment step, you will need **samtools** and ideally the **STAR** aligner.

we recommend installing umite using pip:
...
if this has issuses you can try using `python -m pip install .` to install via the local python interpreter instead. You can also install the `pytest` suite by calling `pip install .[dev]`.
...
for a detailed description of the algorithms underlying umiextract and umicount, as well as implementation details and a full description of command line arguments and their usage, see the attached wiki page: [umite algorithmic and implementation details](https://github.com/leoforster/umite/wiki/umite-algorithmic-and-implementation-details)

### Command-line reference

for completeness-sake, a command line reference is provided here. However, we recommend consulting the linked [wiki page](https://github.com/leoforster/umite/wiki/umite-algorithmic-and-implementation-details) for detailed explanation of individual command-line arguments and their function.
