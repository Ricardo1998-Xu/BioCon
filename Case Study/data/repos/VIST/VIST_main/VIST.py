...
class VIST(nn.Module):
    def __init__(self, inputDim, Nnodes, latent_dim, time_dim, fixStd = False, fixVal = 0.5):
        super().__init__()


        self.latentMean = latent_module_mean(inputDim, Nnodes, latent_dim)
        self.latentStd = latent_module_std(inputDim, Nnodes, latent_dim)
        self.getGamma = get_gamma_params(inputDim, Nnodes, latent_dim + time_dim - 1)              # time 2 latent dim
        self.getGammaS = get_gamma_theta(inputDim, Nnodes, latent_dim + time_dim - 1)              # time 2 latent dim
        self.bernoulli = bernoulli_sample(inputDim, Nnodes, latent_dim + time_dim - 1)             # time 2 latent dim
        ...
        self.fixStd = fixStd
        self.fixVal = fixVal
        # need to concatenate output of expand time and latent mean for computing final log likelihood
    def forward(self, z):
        ...
        '''
        for each cell, get rho
        '''
        rho = self.getGamma(latent_state)
        
        '''
        and the gene specific, cell-specific dispersion
        '''
...
def sample_wasserstein(distribution, sample, dirNum):
    ...
    # sampled_directions = sampled_directions / torch.linalg.norm(sampled_directions, dim = 0)
...
def train_net(input_anndata, 
    ...
    NvarGenes = input_anndata.shape[1]
    time_dim = 1
