...
from model import CodeGen as Model
...
from transformers import (WEIGHTS_NAME, get_linear_schedule_with_warmup, AutoConfig, AutoModelForSequenceClassification,
                          AutoTokenizer,T5Config, T5ForConditionalGeneration,
                          RobertaConfig, RobertaForSequenceClassification, RobertaTokenizer, CodeGenConfig,
                          CodeGenModel)
...
def convert_examples_to_features(js, tokenizer, args):
    ...
    text1 = ' '.join(js['sentence'].split())
    text2 = ' '.join(js['code'].split())
...
class SoftwareDataset(Dataset):
    def __init__(self, tokenizer, args, file_path):
        self.software_examples = []
        with open(file_path) as f:
            for line in f:
                js = json.loads(line.strip())
                statements = js["statements"]
                features = [convert_examples_to_features(s, tokenizer, args) for s in statements]

                self.software_examples.append({
                    "features": features,
                    "label": js["label"],
                    "ratio": js.get("consistency_ratio", None),
                    "software": js["software"],
                    "variant": js["variant"]
                })

    def __len__(self):
        return len(self.software_examples)

    def __getitem__(self, i):
        return self.software_examples[i]
...
def aggregate(probs, method="max", threshold=0.5):
    if method == "max":
        return max(probs)
    elif method == "mean":
...
def test(args, model, tokenizer):
    ...
    eval_dataset = SoftwareDataset(tokenizer, args, args.test_data_file)
    ...
    for item in eval_dataset:
        ...
        with torch.no_grad():
            for feat in features:
                input_ids = torch.tensor(feat.input_ids).unsqueeze(0).to(args.device)
                prob, _ = model(input_ids)
                p_cons = prob[0][1].item()
                p_incons = 1 - p_cons

                probs.append(p_incons)
            # ===== software-level aggregate =====
            p_S = aggregate(probs, method="mean", threshold=threshold)

            pred_label = 0 if p_S > threshold else 1

            y_true.append(gt_label)
            y_pred.append(pred_label)

            if gt_ratio is not None:
                pred_inconsistency = aggregate(probs, method="ratio", threshold=threshold)
                gt_inconsistency = 1 - gt_ratio

                ratio_true.append(gt_inconsistency)
                ratio_pred.append(pred_inconsistency)
    ...
    # ===== MAE, RMSE =====
    mae = mean_absolute_error(ratio_true, ratio_pred) if len(ratio_true) > 0 else None
    rmse = np.sqrt(mean_squared_error(ratio_true, ratio_pred)) if ratio_true else None
...
def main():
    ...
    parser.add_argument("--test_data_file", default='./data/BioCon/software_test.jsonl', type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")

    parser.add_argument("--output_dir", default='./weights/codegen/FL-weight=5-lr0.00002-seed{seed}', type=str,
                        help="The output directory where the model predictions and checkpoints will be written.")
    parser.add_argument('--results_path', type=str,
                        default='./results/codegen/FL-weight=5-lr0.00002-seed{seed}-software', help='save results path')
